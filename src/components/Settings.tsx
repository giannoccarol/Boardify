        ...
        <div className="settings-scroll nice-scroll" key={panel}>
          <motion.div initial={reduce ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={soft} className="settings-content">
            ...
            {panel === "privacy" && <>
              ...
              <Section title={t("settings.history")}><Row label={t("settings.keepUnused")} hint={t("settings.keepUnusedHint")}><select aria-label={t("settings.keepUnused")} className="settings-select" value={settings.autoDeleteDays} onChange={(e) => set("autoDeleteDays", Number(e.target.value))}><option value={0}>{t("settings.forever")}</option><option value={7}>{t("settings.days7")}</option><option value={30}>{t("settings.days30")}</option><option value={90}>{t("settings.days90")}</option></select></Row><Row label={t("settings.maxItems")} hint={t("settings.maxItemsHint")}><select aria-label={t("settings.maxItems")} className="settings-select" value={settings.maxItems} onChange={(e) => set("maxItems", Number(e.target.value))}><option value={0}>{t("settings.unlimited")}</option><option value={100}>100</option><option value={500}>500</option><option value={1000}>1000</option></select></Row><div className="ignored-apps"><label htmlFor="ignored-apps">{t("settings.excludedApps")}</label><p>{t("settings.excludedAppsHint")}</p><textarea id="ignored-apps" value={settings.ignoredApps} onChange={(e) => set("ignoredApps", e.target.value)} rows={3} placeholder={t("settings.excludedPlaceholder")} spellCheck={false} /></div></Section>
              ...
              <Section title={t("settings.manageData")}><Row label={t("settings.resetPrefs")} hint={t("settings.resetPrefsHint")}><button className="secondary-button" onClick={() => { setConfirmAction("reset"); setFeedback(""); }}><RotateCcw size={14} />{t("settings.reset")}</button></Row>{isTauri() && <Row label={t("settings.clearHistory")} hint={t("settings.clearHistoryHint")}><button className="danger-button" onClick={() => { setConfirmAction("clear"); setFeedback(""); }}><Trash2 size={14} />{t("settings.clear")}</button></Row>}{isTauri() && <Row label={t("settings.backup")} hint={t("settings.backupHint")}><button className="secondary-button" disabled={busy} onClick={async () => { setBusy(true); try { const p = await useBoardify.getState().backupNow(); setFeedback(p ? `${t("settings.backupDone")} ${p}` : t("settings.actionFailed")); } catch { setFeedback(t("settings.actionFailed")); } finally { setBusy(false); } }}><HardDrive size={14} />{t("settings.backup")}</button></Row>}{isTauri() && <Row label={t("settings.exportClips")} hint={t("settings.exportHint")}><button className="secondary-button" disabled={busy} onClick={async () => { setBusy(true); try { const n = await useBoardify.getState().exportToClipboard(); setFeedback(t("settings.exported", { count: n })); } catch { setFeedback(t("settings.actionFailed")); } finally { setBusy(false); } }}><Clipboard size={14} />{t("settings.exportClips")}</button></Row>}</Section>
              {isTauri() && <Section title={t("settings.importClips")}><div className="ignored-apps"><p>{t("settings.importHint")}</p><textarea aria-label={t("settings.importClips")} rows={3} placeholder={t("settings.importPlaceholder")} spellCheck={false} value={importJson} onChange={(e) => setImportJson(e.target.value)} /><div><button className="secondary-button" disabled={busy || !importJson.trim()} onClick={async () => { setBusy(true); try { const r = await useBoardify.getState().importBackupJson(importJson); setImportJson(""); setFeedback(t("settings.imported", { imported: r.imported, skipped: r.skipped })); } catch { setFeedback(t("settings.actionFailed")); } finally { setBusy(false); } }}>{t("settings.importButton")}</button></div></div></Section>}
